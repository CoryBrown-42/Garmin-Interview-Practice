...
export default function App() {
  ...
  const checkSolution = async () => {
    setOutput("✨ Reviewing code for performance and correctness...");
    const system = `You are a Senior Unity Tech Artist Interviewer. 
    Evaluate the following code for: \"${exercise.prompt}\". 
    ...
    if (result) {
      ...
      setOutput("⚠️ Error connecting to reviewer. Please try again.");
    }
  };
  ...
  const generateCustomExercise = async () => {
    ...
    setIsAiLoading(true);
    const system = `Generate a new technical artist interview coding exercise based on the following topic. 
      ...
      {showAiModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <p className="text-slate-400 text-sm mb-4">Enter a Garmin-specific topic to generate a new exercise (e.g. "GPS Distance Math", "Aviation PFD UI Optimization", "Golf Swing Physics")</p>
            <textarea 
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex-1 bg-slate-950 rounded-xl p-6 border border-slate-800 shadow-inner overflow-auto max-h-[400px] min-h-[200px]">
            ...
            <div className={`font-mono text-sm whitespace-pre-wrap leading-relaxed ${output.includes("✨") ? "text-emerald-300" : "text-slate-300"}`}>
              {output || "Select an action or check the code..."}
            </div>
          </div>
        </div>
